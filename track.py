...
def cmd_push_que(args) -> None:
    """Push Garmin cardio + daily wellness into the Que app (one batched request)."""
    ...
    try:
        r = push_all(days=days, resend=resend)
    except QueNotConfigured as e:
        print(f"Que push: skipped — {e}")
        return
    s, w = r["activities"], r["wellness"]
    mode = "1 batched request" if r["batched"] else "per-item (older Que server)"
    print(f"Que push ({mode}): {s['sent']} activities + {w['sent']} wellness days sent, "
          f"{s['skipped'] + w['skipped']} skipped, {s['failed'] + w['failed']} failed.")
